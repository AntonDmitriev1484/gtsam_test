...
def load_tum_trajectory(filepath):
    """Load TUM trajectory file: WITH SPACES"""
    poses = []
    quats = []
...
def draw_axes(ax, origin, R, length=0.25):
    x_axis = R[:, 0] * length
    y_axis = R[:, 1] * length
    z_axis = R[:, 2] * length
...
def plot_trajectory_comparison(synthetic_name, stride=0, show=False):
    base = synthetic_name
    uwb = f"{base}_uwb"

    # File paths
    est1_path = f"{base}/est.txt"
    gt1_path = f"{base}/slam.txt"
    est2_path = f"{uwb}/est.txt"
    gt2_path = f"{uwb}/slam.txt"
    ...
    plot_trajectory(ax1, est1, "Est 1", gt1, "GT 1", "blue", "green", quat1, quat_gt1, stride)

    # Plot 2
    fig2 = plt.figure()
    ax2 = fig2.add_subplot(111, projection='3d')
    ax2.set_title("Plot 2: Estimated (UWB + GT + IMU) vs Ground Truth")
    plot_trajectory(ax2, est2, "Est 2", gt2, "GT 2", "blue", "green", quat2, quat_gt2, stride)
...
def main():
    ...
    gt1_path = f"./{args.synthetic}/slam.txt"
        # gt1_path = f"./{args.synthetic}/slam_interp.txt"
    est2_path = f"./{args.synthetic}_uwb/est.txt"
    gt2_path = f"./{args.synthetic}_uwb/slam.txt"
        # gt2_path = f"./{args.synthetic}/slam_interp.txt"
