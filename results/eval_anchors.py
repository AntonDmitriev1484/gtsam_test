...
def plot_anchors(trialname, fig, ax, show_optimization):
    ...
    all_json = json.load(open(f"/home/antond2/ws/post/out/{trialname}_post/all.json", 'r'))
    for j in all_json:
        if j["type"] == "synth_visual_anchor_prior":
            prior = np.array(j["T_body_world"]).reshape((4,4))[:3,3]
            if str(j["src"]) not in synth_priors:
                synth_priors[str(j["src"])] = [prior]
            else:
                synth_priors[str(j["src"])].append(prior)
    ...
    for id in est_anchors.keys():
        ...
        for p in synth_priors[id]:
            ax.scatter(p[0], p[1], p[2], color="purple", s=80)
